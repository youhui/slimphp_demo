...
class DbOperations {
	...
	public function userLogin($email, $password) {
		if ($this->isEmailExist($email)) {
			$encrypted_password = $this->getUsersPasswordByEmail($email);
			if (strcmp(sha1($password), $encrypted_password)) {
	...
	public function getAllUsers() {
		$stmt = $this->con->prepare("SELECT id, email, name, school, password FROM users;");
		$stmt->execute();
		$stmt->bind_result($id, $email, $name, $school, $password);
		$users = array();
		while ($stmt->fetch()) {
			$user = array();
			$user['id'] = $id;
	...
	public function updatePassword($currentpassword, $newpassword, $email) {
		...
		if (strcmp(sha1($password), $encrypted_password)) {
			...
			$encrypted_pwd = sha1($newpassword);
